...
database = dict()
with open("MultiWOZ2.4-main/data/mwz24/MULTIWOZ2.4/restaurant_db.json", "r") as f:
    database["restaurant"] = json.load(f)
with open("MultiWOZ2.4-main/data/mwz24/MULTIWOZ2.4/hotel_db.json", "r") as f:
    database["hotel"] = json.load(f)
with open("MultiWOZ2.4-main/data/mwz24/MULTIWOZ2.4/attraction_db.json", "r") as f:
    database["attraction"] = json.load(f)
with open("MultiWOZ2.4-main/data/mwz24/MULTIWOZ2.4/train_db.json", "r") as f:
    database["train"] = json.load(f)
with open("MultiWOZ2.4-main/data/mwz24/MULTIWOZ2.4/taxi_db.json", "r") as f:
    db_str = f.read()
...
def query_basic(q, domain, max_retrieval=10, fuzzy_ratio=80):
    ...
    query = {slot: value for slot, value in query.items() if value.lower() != "any" and value.lower() != "dontcare"}
    valid_items = []
    for database_item in database[domain]:
        ...
        if valid:
            if len(valid_items) == max_retrieval:
                return json.dumps({"result": valid_items, "message": "Failure! Too many retrieved results! Please query more accurately!"})
